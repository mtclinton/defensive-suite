// Package runner abstracts external command execution so the enumerator can
// shell out to `bpftool ... -j` in production and run against canned JSON in
// tests. It is the seam that keeps the portable enumeration path fully testable
...
// without a Go error because tools like `bpftool` can exit non-zero to report a
// missing object without that being an operational failure.
type Result struct {
	Stdout   string
	Stderr   string
	ExitCode int
...
// callers degrade to an informational "tool absent" finding instead of failing.
