...
// npm in production and run against canned output in tests. Copied from
// authwatch — the suite reproduces shared contracts rather than importing across
// modules, keeping each tool an independent, offline-buildable module.
package runner
...
// without a Go error because tools like `npm audit` exit non-zero to report findings.
type Result struct {
	Stdout   string
	Stderr   string
	ExitCode int
