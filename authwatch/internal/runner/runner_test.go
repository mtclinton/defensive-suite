...
func TestExecNotFound(t *testing.T) {
	_, err := Exec{}.Run(context.Background(), "authwatch-no-such-binary-xyz")
...
func TestFakeResponsesAndCalls(t *testing.T) {
	f := &Fake{Responses: map[string]Result{"rpm -qf /x": {Stdout: "pkg-1", ExitCode: 0}}}
	res, err := f.Run(context.Background(), "rpm", "-qf", "/x")
	if err != nil {
		t.Fatal(err)
	}
	if res.Stdout != "pkg-1" {
	...
	if len(f.Calls) != 1 || f.Calls[0] != "rpm -qf /x" {
...
func TestFakeInjectedError(t *testing.T) {
	...
	f := &Fake{Errs: map[string]error{"aide --check": sentinel}}
	if _, err := f.Run(context.Background(), "aide", "--check"); !errors.Is(err, sentinel) {
