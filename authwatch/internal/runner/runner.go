...
// rpm/dpkg/aide/auditctl in production and run against canned output in tests.
package runner
...
// without a Go error because tools like `rpm -V` exit non-zero to report findings.
type Result struct {
	Stdout   string
	Stderr   string
	ExitCode int
