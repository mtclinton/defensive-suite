...
// ss/ip in production and run against canned output in tests. egresswatch's core
// parsers are pure functions over text, so the live runner is only used to
// gather that text (e.g. `ss -tunp`) — the decision logic never shells out.
package runner
...
// without a Go error because some tools exit non-zero to report state.
type Result struct {
	Stdout   string
	Stderr   string
	ExitCode int
