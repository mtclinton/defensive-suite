...
func TestExecNotFound(t *testing.T) {
	_, err := Exec{}.Run(context.Background(), "posturescan-no-such-binary-xyz")
...
func TestFakeResponsesAndCalls(t *testing.T) {
	f := &Fake{Responses: map[string]Result{"sysctl -a": {Stdout: "k = v", ExitCode: 0}}}
	res, err := f.Run(context.Background(), "sysctl", "-a")
	if err != nil {
		t.Fatal(err)
	}
	if res.Stdout != "k = v" {
	...
	if len(f.Calls) != 1 || f.Calls[0] != "sysctl -a" {
...
func TestFakeInjectedError(t *testing.T) {
	...
	f := &Fake{Errs: map[string]error{"oscap xccdf eval": sentinel}}
	if _, err := f.Run(context.Background(), "oscap", "xccdf", "eval"); !errors.Is(err, sentinel) {
