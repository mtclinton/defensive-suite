...
// lynis/oscap/systemd-analyze/sysctl in production and run against canned output
// in tests.
package runner
...
// without a Go error because tools like `oscap` exit non-zero to report findings.
type Result struct {
	Stdout   string
	Stderr   string
	ExitCode int
